...
class MWComanda
{
    ...
    function MWVerificarCredenciales(Request $request,Response $response,$next)
    {
        ...
        switch($data->tipo)
        {
            ...
            case "mozo":
            if(starts_with($request->getUri()->getPath(),'/Pedidos/') && $request->getUri()->getPath()!='/Pedidos/TiempoEstimado' || starts_with($request->getUri()->getPath(), '/Menu/') && $request->getMethod() == "GET" || starts_with($request->getUri()->getPath(), '/Mesa/') && $request->getMethod() == "GET" || starts_with($request->getUri()->getPath(), '/Mesa/') && $request->getMethod() == "GET")
            {
                $response = $next($request,$response);
            }
            else
